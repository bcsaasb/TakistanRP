	...
	if user_id then
		if parseInt(valor) > 0 then
			if vRP.tryDeposit(user_id,parseInt(valor)) then
				TriggerClientEvent("Notify",source,"sucesso","Depositou <b>R$"..vRP.format(parseInt(valor)).." reais</b> em sua conta bancária.")
			else
				TriggerClientEvent("Notify",source,"negado","Não possui <b>R$"..vRP.format(parseInt(valor)).." reais</b> em sua carteira.")
			end
		end
	end
end
		...
		if valor then
			if parseInt(valor) > 0 then
				if vRP.tryWithdraw(user_id,parseInt(valor)) then
					TriggerClientEvent("Notify",source,"sucesso","Sacou <b>R$"..vRP.format(parseInt(valor)).." reais</b> de sua conta bancária.")
				else
					TriggerClientEvent("Notify",source,"negado","Não possui <b>R$"..vRP.format(parseInt(valor)).." reais</b> em sua conta bancária.")
				end
			end
		else
			if caixas[user_id] == 0 or not caixas[user_id] then
				if vRP.tryWithdraw(user_id,1000) then
					caixas[user_id] = 600
					TriggerClientEvent("Notify",source,"sucesso","Sacou <b>R$1.000 reais</b> de sua conta bancária.")
				else
					TriggerClientEvent("Notify",source,"negado","Não possui <b>R$1.000 reais</b> em sua conta bancária.")
				end
			else
