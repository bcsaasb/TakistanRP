...
AddEventHandler('bank:deposit', function(amount)
	...
	if(tonumber(amount))then
		if(vRP.tryPayment(user_id, amount))then
			...
			TriggerClientEvent("Notify",thePlayer,"sucesso","Depositou <b>R$"..vRP.format(parseInt(amount)).." reais</b> na sua conta bancária.")
		else
...
AddEventHandler('bank:withdraw', function(amount)
	...
	if(tonumber(amount))then	
		...
		if(amount > 0 and amount <= bankMoney)then
			vRP.setBankMoney(user_id, bankMoney-amount)
			vRP.setMoney(user_id, walletMoney+amount)
			TriggerClientEvent("Notify",thePlayer,"sucesso","Sacou <b>R$"..vRP.format(parseInt(amount)).." reais</b> da sua conta bancária.")
		else
			TriggerClientEvent("Notify",thePlayer,"negado","Não possui <b>R$"..vRP.format(parseInt(amount)).." reais</b> em sua conta bancária.")
		end
	else
		TriggerClientEvent("Notify",thePlayer,"negado","Número inválido.")
	end
			...
			if(thePlayer == theTarget)then
				...
				if(tonumber(amount) and tonumber(amount) > 0 and amount ~= "" and amount ~= nil)then
					...
					if(bankMoney >= amount)then
						...
						TriggerClientEvent("Notify",thePlayer,"sucesso","Você transferiu <b>R$"..vRP.format(parseInt(amount)).." reais</b> para" ..GetPlayerName(theTarget))
						TriggerClientEvent("Notify",theTarget,"sucesso","Transferencia Efetuada <b>R$"..vRP.format(parseInt(amount)).." reais</b> para" ..GetPlayerName(thePlayer))
					else
						TriggerClientEvent("Notify",thePlayer,"negado","Você não tem dinheiro suficiente.")
					end
				else
