	...
	if user_id then
		...
		if business then
			...
			menu["Investir"] = { function(player,choice)
				...
				if amount > 0 then
					if vRP.tryPayment(user_id,amount) then
						...
						TriggerClientEvent("Notify",player,"sucesso","Investiu <b>R$"..vRP.format(amount).." reais</b> e adicionou <b>R$"..vRP.format(parseInt(amount*0.5)).."</b> ao capital.")
					else
			...
			menu["Lavagem"] = { function(player,choice)
				...
				if parseInt(amount) > 0 and parseInt(amount) <= restante then
					if vRP.tryGetInventoryItem(user_id,"dinheirosujo",parseInt(amount)) then
						...
						TriggerClientEvent("Notify",player,"sucesso","Lavou <b>R$"..vRP.format(amount).." reais sujos</b> e recebeu <b>R$"..vRP.format(parseInt(amount*("0."..random))).." reais</b> limpos.")
					else
			...
			menu["Comprar"] = { function(player,choice)
				local ok = vRP.request(source,"Deseja compra da empresa por <b>$2.000.000</b> reais?",30)
				if ok then
