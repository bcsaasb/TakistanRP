...
AddEventHandler("vRP:playerJoin",function(user_id,source,name)
	vRP.execute("vRP/money_init_user",{ user_id = user_id, wallet = 3000, bank = 25000 , xplevel = 0})
	local tmp = vRP.getUserTmpTable(user_id)
	if tmp then
...
AddEventHandler("vRP:playerLeave",function(user_id,source)
	...
	if tmp and tmp.wallet and tmp.bank then
		vRP.execute("vRP/set_money",{ user_id = user_id, wallet = tmp.wallet, bank = tmp.bank , xplevel = tmp.xplevel})
	end
end)
...
AddEventHandler("vRP:save",function()
	for k,v in pairs(vRP.user_tmp_tables) do
		if v.wallet and v.bank then
			vRP.execute("vRP/set_money",{ user_id = k, wallet = v.wallet, bank = v.bank , xplevel = v.xplevel})
		end
	end
end)