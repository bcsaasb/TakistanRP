		...
		if vehDetails ~= nil then
			if rows[1].detido == 1 then
				...
				if parseInt(rows[1].time) == 0 then
					...
					ok = vRP.request(source, 'Veículo roubado, deseja acionar o seguro pagando <b>$'..vRP.format(value)..'</b> reais?', 60)					
				else
					value = parseInt(vehDetails[2]*detainedValue)
					ok = vRP.request(source, 'Veículo detido, deseja liberar pagando <b>$'..vRP.format(value)..'</b> reais?', 60)					
				end				
				if ok then
