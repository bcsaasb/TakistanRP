...
function emP.checkPayment()
	...
	if user_id then
		...
		if vRP.tryGetInventoryItem(user_id,"cocaina",quantidade[source]) then 
			vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(550,700)*quantidade[source]+(#policia*50))
			vRPclient._playAnim(source,true,{{"mp_common","givetake1_a"}}, false)
			quantidade[source] = nil
		end

		if vRP.tryGetInventoryItem(user_id,"metanfetamina",quantidade[source]) then 
			vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(450,600)*quantidade[source]+(#policia*50))
			vRPclient._playAnim(source,true,{{"mp_common","givetake1_a"}}, false)
			quantidade[source] = nil
		end

		if vRP.tryGetInventoryItem(user_id,"maconha",quantidade[source]) then 
			vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(350,500)*quantidade[source]+(#policia*50))
			vRPclient._playAnim(source,true,{{"mp_common","givetake1_a"}}, false)
			quantidade[source] = nil
		end
