...
Config.Language = {
    ...
    timedout = "Desconectado por não estar respondendo.",
    wlonly = "Você precisa estar na whitelist para entrar no servidor",
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for k,v in ipairs(self.QueueList) do
        if tmp.priority then
            ...
            if _pos then
                self:DebugPrint(string_format("%s[%s] possui prioridade na fila e foi colocado %d/%d na fila", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        _pos = self:GetSize() + 1
        self:DebugPrint(string_format("%s[%s] está %d/%d na fila", tmp.name, ids[1], _pos, queueCount))
    end
...
Citizen.CreateThread(function()
    local function playerConnect(name, setKickReason, deferrals)
        ...
        local reason = "Você foi kikado entrando na fila"
        ...
        if Queue:IsInQueue(ids) then
            ...
            Queue:DebugPrint(string_format("%s[%s] reentrou na fila depois de ter cancelado", name, ids[1]))
        else
        ...
        if Queue:NotFull(true) then
            ...
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] está entrando no servidor")
			TriggerEvent("queue:playerConnecting", src, ids, name, setKickReason, deferrals)
    ...
    local function checkTimeOuts()
        ...
        while i <= Queue:GetSize() do
            ...
            if not data.ids or not data.name or not data.firstconnect or data.priority == nil or not data.source then
                ...
                Queue:DebugPrint(tostring(data.name) .. "[" .. tostring(data.ids[1]) .. "] foi removido da fila devido problemas no banco")
            ...
            elseif (data.timeout >= 300) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                ...
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] foi removido da fila por conta de um timeout")
            else
                i = i + 1
            end
        end
