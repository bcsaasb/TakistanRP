	...
	if user_id and price > 0 then
		if vRP.tryPayment(user_id,price) then
			if galao then
				...
				TriggerClientEvent("Notify",source,"sucesso","Pagou <b>R$"..vRP.format(price).." reais</b> pelo <b>Galão</b>.")
			else
				TriggerClientEvent("Notify",source,"sucesso","Pagou <b>R$"..vRP.format(price).." reais</b> em combustível.")
			end
		else
